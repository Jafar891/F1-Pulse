...
# Provides retry logic with fixed-delay backoff and client-error short-circuit.
# =============================================================================
...
def fetch_with_retry(
    ...
    """
    GET a URL with retry and fixed-delay backoff. Expects a JSON list response.

    Retry behaviour
    ---------------
    - Retries on Timeout and generic RequestException.
    - Short-circuits immediately on 4xx client errors (no point retrying).
    - Retries on 429 Too Many Requests after the standard delay.
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # Short-circuit on unrecoverable client errors
            if status in (400, 401, 403, 404):
                break
            # 429 and 5xx fall through to retry with delay
